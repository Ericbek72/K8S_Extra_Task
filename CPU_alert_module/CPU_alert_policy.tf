...
resource "google_monitoring_alert_policy" "cpu_alert_policy" {
  ...
  display_name = var.cpu_alert_pol
  combiner     = "OR"

  conditions {
    display_name = var.cpu_alert_pol_cond
    condition_threshold {
      filter     = var.cpu-threshold-filter
      duration   = var.cpu-wait-time
      threshold_value = var.cpu-threshold
      comparison = var.cpu-alert-if-th-above
      aggregations {
